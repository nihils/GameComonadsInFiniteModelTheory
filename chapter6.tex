...
\begin{proof}
\begin{enumerate}[label=(\arabic*)]
Throughout we will need to distinguish between the maps associated with $\efcomonad{k}{}$ and $\pcomonad{k}{}$ comonads using superscripts. For example, the counits will be denoted $\epsilon^{\mathbb{E}}$ and $\epsilon^{\mathbb{T}}$ respectively.  
...
\end{proof}
\end{prop}
A result by Gabbay shows \cite{Gabbay1981} that the full modal fragment of infinitary logic is contained within the two-variable fragment of infinitary logic. We prove the same result, using our comonadic formulation.
\begin{prop}
