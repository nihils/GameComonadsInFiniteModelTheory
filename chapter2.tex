...
\begin{defn}
A \textit{signature} or \textit{vocabulary}, denoted with lowercase greek letters like $\sigma$, is a set of constant symbols $c_{1},c_{2},\dots$, function symbols $f_{1},f_{2},\dots$, and relation symbols $R_{1},R_{2},\dots$ where every function symbol and relation symbol has an associated arity. 
\end{defn}
\begin{defn}
...
\end{itemize}
\end{defn}
This also leads to very fine notion of equivalence between two $\sigma$-structures. 
\begin{defn}
A bijective $\sigma$-morphism $f:A \longrightarrow B$ whose inverse $g:B \longrightarrow A$ is also a $\sigma$-morphism is a \textit{isomorphism}. Two $\sigma$-structures $A,B$ are \textit{isomorphic}, denoted $A \cong B$, if there exists an isomorphism between them. 
\end{defn}
...
such that for $A,B \in C$:
$$A \cong B \Rightarrow P(A) = P(B)$$ 
where $\mathcal{C}$ is a class of all finite $\sigma$-structures.
\end{defn}
Morally, $P(A) = 1$ asserts that some property is true of the structure $A$. \\ 
A ``back-and-forth'' game between two structures $A,B$ is a two-player, conventionally called Spoiler and Duplicator, perfect-information game. A round of the game is first a move by Spoiler chooses an element of $A$ or $B$, and Duplicator must respond with a ``corresponding'' element in the other structure. For a given a logic. 
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item For every object $A \in \mathcal{C}$, there exists a morphism $id_{A}:A \longrightarrow A$ such that $id_{A} \circ f = f$ and $g \circ id_{A} = g$ for every $f:B \longrightarrow A$, $g:A \longrightarrow C$. 
    \item For morphisms $f:A \longrightarrow B$, $g:B \longrightarrow C$ and $h:C \longrightarrow D$, $h \circ (g \circ f) = (h \circ g) \circ f$. 
\end{enumerate}
We can rephrase the condition on $id_{A}:A \longrightarrow A$ by stating that the following diagrams commute for all $A \in \mathcal{C}$:
\begin{equation*}
\bfig
    \ptriangle[B`A`A;f`f`id_{A}]
\efig
\bfig
    \dtriangle[A`A`C;id_{A}`g`g]
\efig
\end{equation*}
...
\begin{enumerate}[label=(\arabic*)]
    \item  For every object $A \in \mathcal{C}$, $F(id_{A}) = id_{F(A)}$ 
    \item  For morphisms $f:A \longrightarrow B$ and $g:B \longrightarrow C$, $F(g \circ f) = F(g) \circ F(f)$.  
\end{enumerate}
\end{defn}
\begin{defn}
A functor $F:\mathcal{C} \longrightarrow \mathcal{C}$ from a category to itself is callend an \textit{endofunctor} 
\end{defn}
...
\begin{itemize}
    ...
    \end{equation}
\end{itemize}
The composition law is the same as in $\mathcal{C}$ and the identity on $(A,\alpha)$ is just the $\mathcal{C}$-morphism $id_{A}:A \longrightarrow A$.
\end{defn}
