...
\begin{proof}
Fix a linear order $<$ of $B$. \\
$S_{f} \subseteq S$ The strategy is to construct an appropriate $f$. Define $\pi_{1}:\Gamma_{k}(A,B) \longrightarrow \efcomonad{k}{A}$ by $\pi_{1}:[(a_{1},b_{1}),\dots,(a_{n},b_{n})] \longrightarrow [a_{1},\dots,a_{n}]$ and $\pi_{2}:\Gamma_{k}(A,B) \longrightarrow \efcomonad{k}{B}$ by $\pi_{2}:[(a_{1},b_{1}),\dots,(a_{n},b_{n})] \longrightarrow [b_{1},\dots,b_{n}]$. Suppose $s = [a_{1},\dots,a_{n}] \in \efcomonad{k}{A}$, consider the subset $Z = \{\chi \in \Gamma_{k}(A,B):\pi_{1}(\chi) = s\}$. By (\ref{eq:S1st}) and applying (\ref{eq:S2nd}) $n$ times (i.e. for each $a_{i}$), there exists a $\chi \in S$ such that $\pi_{1}(\chi) = s$. Therefore, $Z$ is non-empty and $\epsilon_{B}(\pi_{2}(Z)) = \{b \in B: \exists \chi \in Z \text{ with } \epsilon_{B}(\pi_{2}(\chi)) = b\} \subseteq B$ is non-empty. Let $f(s) = b'$ where $b'$ is the $<$-least element of this set. Clearly, by construction, $S_{f} = S$ \\
...
\begin{defn}
A position $\chi = [(a_{1},b_{1}),\dots,(a_{n},b_{n})] \in \Gamma_{k}(A,B)$ is \textit{winning for Duplicator} if the map $\chi:a_{i} \longmapsto b_{i}$ is a paritial $\sigma$-morphism from $A$ to $B$.  
